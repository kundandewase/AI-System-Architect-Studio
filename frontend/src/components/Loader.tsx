...
export default function Loader() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[450px] w-full p-8 text-center">
      {/* Outer Glow Ring */}
      <div className="relative w-24 h-24 mb-8">
        ...
        <div className="absolute inset-2 rounded-full border border-zinc-800 flex items-center justify-center bg-zinc-950">
      ...
      <div className="max-w-md w-full px-4">
        <div className="h-1.5 w-full bg-zinc-900 rounded-full overflow-hidden mb-6 border border-zinc-800/50">
          <div 
        ...
        <div className="mt-4 flex flex-col items-start gap-2 max-w-xs mx-auto border-t border-zinc-900 pt-6 text-left">
          {STEPS.map((step, idx) => {
            ...
            return (
              <div key={idx} className="flex items-center gap-2.5 text-xs">
                {isCompleted ? (
                  ...
                  <span className="w-1.5 h-1.5 rounded-full bg-zinc-800 mx-1" />
                )}
                <span className={isCompleted ? 'text-zinc-500 line-through' : isCurrent ? 'text-zinc-200 font-medium' : 'text-zinc-600'}>
                  {step.replace('...', '')}
                </span>
              </div>
            );
